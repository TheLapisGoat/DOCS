...
#define CLIENT_PACKET_LENGTH 1024
...
public:
    DPDKClient(uint16_t port, struct rte_mempool* pool) :
        port_id(port), mbuf_pool(pool) {

        // Predefined messages for transmission
        messages = {
            "Hello, DPDK Server!",
            "This is a test packet",
            "DPDK networking is powerful",
    ...
    void send_packets() {
        for (const auto& message : messages) {
            ...
            char* payload = rte_pktmbuf_append(tx_bufs[0], message.length() + 1);
            strcpy(payload, message.c_str());

            uint16_t nb_tx = rte_eth_tx_burst(port_id, 0, tx_bufs, 1);

            if (nb_tx == 0) {
                std::cerr << "Packet transmission failed" << std::endl;
    ...
    void wait_for_acknowledgments(int timeout_seconds = 5) {
        ...
        while (true) {
            ...
            if (nb_rx > 0) {
                for (uint16_t i = 0; i < nb_rx; i++) {
                    ...
                    std::cout << "Received ACK: " << payload << std::endl;
...
int main(int argc, char** argv) {
    ...
    struct rte_mempool* mbuf_pool = rte_pktmbuf_pool_create(
        "CLIENT_MBUF_POOL", NUM_MBUFS, MBUF_CACHE_SIZE, 0,
    ...
    // Use the second available port for client
    DPDKClient client(1, mbuf_pool);
    ...
    std::cout << "DPDK Client started. Sending packets..." << std::endl;

    // Send packets
    client.send_packets();

    // Wait for acknowledgments
    client.wait_for_acknowledgments();
